...
export default function DeudasVisualPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) return;
      const { data } = await supabase.from("deudas").select("*").eq("user_id", user.id).order("total_pendiente", { ascending: false });
  ...
  // Snowball: sorted smallest to largest
  const snowball = [...deudas].sort((a, b) => a.total_pendiente - b.total_pendiente);
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-6">
          ...
          {/* Snowball strategy */}
          <div className="bg-white rounded-2xl border border-[#ffb8e0] p-6">
            <h2 className="font-semibold text-[#1a1a2e] mb-1">❄️ Estrategia bola de nieve</h2>
            <p className="text-xs text-[#1a1a2e]/50 mb-5">
              Paga el mínimo en todas y enfoca el dinero extra en la deuda más pequeña. Al pagarla, ese dinero pasa a la siguiente. Genera momentum y motivación.
            </p>
            <div className="flex flex-col gap-3">
              {snowball.filter((d) => d.total_pendiente > 0).map((d, i) => (
                <div key={d.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 flex items-center justify-between bg-[#ffedfa] rounded-xl px-4 py-2.5">
                    <p className="text-sm font-medium text-[#1a1a2e]">
                      ...
                      {i === 0 && <span className="ml-2 text-xs text-[#ec7fa9] font-semibold">← empieza aquí</span>}
                    </p>
                    <p className="text-sm font-bold text-[#1a1a2e]">{fmt(d.total_pendiente)}</p>
                  </div>
                </div>
