...
function ListInput({
  items, onAdd, onRemove, placeholder, fmt,
}: {
  items: LineItem[];
  ...
  return (
    <div>
      <div className="flex gap-2 mb-3">
        <input
          value={desc} onChange={(e) => setDesc(e.target.value)} placeholder={placeholder}
        ...
        />
        <input
          type="number" value={val} onChange={(e) => setVal(e.target.value)} placeholder="0"
          onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), add())}
...
export default function DashboardPage() {
  ...
  const [gastosVariablesItems, setGastosVariablesItems] = useState<LineItem[]>([]);

  // Deudas (permanent, not per-month)
  ...
  // Bolsillos (permanent, not per-month)
  ...
  const loadMonth = useCallback(async (m: number, y: number) => {
    ...
    if (data) {
      ...
      setGastosVariablesItems(data.gastos_variables_items ?? []);
    } else {
      setIngresoFijo(""); setIngresosOtros([]); setGastosFijosItems([]); setGastosVariablesItems([]);
    }
    setLoading(false);
  }, []);
  ...
  async function saveData() {
    ...
    await supabase.from("dashboard_mensual").upsert({
      ...
      gastos_variables_items: gastosVariablesItems,
      gastos_fijos: gastosFijosItems.reduce((s, i) => s + i.valor, 0),
      gastos_variables: gastosVariablesItems.reduce((s, i) => s + i.valor, 0),
  ...
  // Calculations
  const totalIngresoFijo = parseFloat(ingresoFijo) || 0;
  const totalIngresosOtros = ingresosOtros.reduce((s, i) => s + i.valor, 0);
  const totalIngresos = totalIngresoFijo + totalIngresosOtros;
  const totalGastosFijos = gastosFijosItems.reduce((s, i) => s + i.valor, 0);
  const totalGastosVariables = gastosVariablesItems.reduce((s, i) => s + i.valor, 0);
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl border border-[#ffb8e0] p-6">
            <h2 className="font-semibold text-[#1a1a2e] mb-5 flex items-center gap-2 text-lg">
              💰 Ingresos del mes
            </h2>
            <div className="flex flex-col gap-5">
              ...
              <div>
                ...
                <ListInput
                  ...
                  placeholder="Ej: Freelance, renta..."
                  fmt={fmt}
                />
              </div>
            </div>
          ...
          {/* ── Gastos fijos ── */}
          <div className="bg-white rounded-2xl border border-[#ffb8e0] p-6">
            <h2 className="font-semibold text-[#1a1a2e] mb-2 flex items-center gap-2 text-lg">
              📌 Gastos fijos
            </h2>
          ...
          <div className="bg-white rounded-2xl border border-[#ffb8e0] p-6">
            ...
            <p className="text-xs text-[#1a1a2e]/40 mb-4">Los que cambian cada mes — comida, transporte, salidas, etc.</p>
            <ListInput
              items={gastosVariablesItems}
              onAdd={(item) => setGastosVariablesItems([...gastosVariablesItems, item])}
              onRemove={(id) => setGastosVariablesItems(gastosVariablesItems.filter((i) => i.id !== id))}
              placeholder="Ej: Mercado, transporte..."
              fmt={fmt}
            />
            <div className="mt-4 pt-4 border-t border-[#ffb8e0] flex justify-between">
              <span className="text-sm font-semibold text-[#1a1a2e]/60">Total gastos variables</span>
              <span className="text-lg font-bold text-red-400">{fmt(totalGastosVariables)}</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[#ffb8e0] p-6">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="font-semibold text-[#1a1a2e] text-lg flex items-center gap-2">💳 Mis deudas</h2>
            ...
            {showDeudaForm && (
              <form onSubmit={addDeuda} className="bg-[#ffedfa] rounded-2xl p-4 mb-4 grid grid-cols-2 gap-3">
                ...
                <div className="col-span-2">
                  <button type="submit" className="w-full bg-[#ec7fa9] text-white font-semibold py-2 rounded-xl text-sm hover:bg-[#d96d97]">
                    Registrar deuda
                  </button>
                </div>
              </form>
            ...
            {deudas.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {deudas.map((d) => (
                  <DeudaRow key={d.id} d={d} onRemove={removeDeuda} fmt={fmt} />
                ))}
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[#ffb8e0] p-6">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="font-semibold text-[#1a1a2e] text-lg flex items-center gap-2">🐷 Bolsillos de ahorro</h2>
            ...
            {showBolsilloForm && (
              <form onSubmit={addBolsillo} className="bg-[#ffedfa] rounded-2xl p-4 mb-4 flex flex-col gap-3">
                ...
                <button type="submit" className="bg-[#ec7fa9] text-white font-semibold py-2 rounded-xl text-sm hover:bg-[#d96d97]">
                  Crear bolsillo 🐷
                </button>
              </form>
            )}
            ...
            {bolsillos.length === 0 ? (
              <div className="text-center py-8 text-[#1a1a2e]/30">
                <p className="text-2xl mb-2">🐷</p>
                <p className="text-sm">Crea tu primer bolsillo de ahorro</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {bolsillos.map((b) => (
                  <BolsilloRow key={b.id} b={b} onAbonar={abonarBolsillo} onRemove={removeBolsillo} fmt={fmt} />
                ))}
              </div>
            )}
          </div>
    ...
    <div className="flex items-center justify-between bg-[#ffedfa] rounded-xl px-4 py-3">
      ...
      <div className="flex items-center gap-4 flex-shrink-0 ml-4">
        <div className="text-right">
          <p className="text-xs text-[#1a1a2e]/40">cuota/mes</p>
          <p className="text-sm font-bold text-[#ec7fa9]">{fmt(d.cuota_mensual)}</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-[#1a1a2e]/40">pendiente</p>
          <p className="text-sm font-bold text-[#1a1a2e]">{d.total_pendiente === 0 ? "✓ Pagada" : fmt(d.total_pendiente)}</p>
        </div>
        <div className="text-right hidden md:block">
          <p className="text-xs text-[#1a1a2e]/40">meses</p>
...
function BolsilloRow({ b, onAbonar, onRemove, fmt }: {
  b: Bolsillo; onAbonar: (id: string, amount: number) => void; onRemove: (id: string) => void; fmt: (n: number) => string;
}) {
    ...
    <div className="bg-[#ffedfa] rounded-2xl p-4">
      ...
      <div className="mb-3">
        <div className="flex justify-between text-xs mb-1">
          <span className="text-[#1a1a2e]/60">{fmt(b.actual)}</span>
          <span className="font-semibold text-[#ec7fa9]">{pct.toFixed(0)}%</span>
        </div>
        <div className="h-2.5 bg-[#ffb8e0] rounded-full overflow-hidden">
      ...
      {pct < 100 ? (
        ...
        <div className="text-center bg-green-50 border border-green-200 rounded-xl py-1.5">
          <p className="text-xs font-semibold text-green-600">🎉 ¡Meta alcanzada!</p>
        </div>
      )}
    </div>
  );
